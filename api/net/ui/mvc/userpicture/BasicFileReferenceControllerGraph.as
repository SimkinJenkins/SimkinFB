...
//				_model.setError(BasicFileReferenceStates.UPLOAD_ERROR, $error.toString());
		...
		protected function completeDataHandler($event:DataEvent):void {
			...
			try {
				response = DataSender.getServerResponse($event.data, DataSenderFormat.XML_DATA_MANAGER, DataSender.DEFULT_INITIAL_TAG);
			} catch($error:Error) {
				response = DataSender.getJSONServerResponse($event.data);
			}
